...
/** Izdarīšanas laiks — "Šodien", "Vakar" vai datums. */
export function doneLabel(iso: string | null, today: string | null) {
  if (!iso) return "";
  const date = iso.slice(0, 10);
...
export function doneCountLabel(n: number) {
  return `${n} ${plural(n, "izdarīts", "izdarīti", "izdarītu")}`;
}
