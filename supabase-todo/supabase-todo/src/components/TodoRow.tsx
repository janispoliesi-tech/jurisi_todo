...
import { archivedLabel, cn, dueInfo } from "@/lib/utils";
...
export default function TodoRow({
  ...
  return (
    <li className="group relative flex items-start gap-2 rounded-xl transition hover:bg-surface-2">
      {/* Atzīmēšana / atjaunošana */}
      <button
        type="button"
        onClick={onToggle}
        aria-pressed={done}
        aria-label={done ? "Atjaunot no arhīva" : "Atzīmēt kā izdarītu"}
        className="grid size-11 shrink-0 place-items-center rounded-xl active:scale-90"
      >
        {done ? (
          <span className="grid size-[22px] place-items-center rounded-full bg-[var(--text-faint)] text-[var(--surface)] transition group-hover:bg-[var(--accent)]">
      ...
      >
        ...
        (done && todo.done_at) ? (
          <span className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1">
            {showCategory && category ? (
              <span
                className="cat-tint cat-text inline-flex items-center gap-1 rounded-md px-1.5 py-0.5 text-[11.5px] font-medium"
            ...
            {done && todo.done_at ? (
              <span className="text-[11.5px] text-faint">
                Pabeigts {archivedLabel(todo.done_at, today).toLowerCase()}
              </span>
            ) : null}
          </span>
        ) : null}
